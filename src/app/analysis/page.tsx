...
function MonthlyReport({
  ...
  const incomeGroups = Object.entries(
    ...
    .map(([name, d]) => ({ name, total: d.total, items: groupByDesc(d.txs) }))
  ...
  const diff = (cur: number, prev: number) => {
    ...
    return {
      d,
      str: `${d >= 0 ? '+' : ''}${formatCurrency(Math.abs(d))} (${pct >= 0 ? '+' : ''}${pct.toFixed(1)}%)`,
      pos: d >= 0,
    }
  }
  ...
  const thStyle: React.CSSProperties = {
    padding: '8px 14px', fontWeight: 800, fontSize: 11,
    color: '#6b7280', background: '#f9fafb',
    borderBottom: '1px solid #e4e9f5',
  }
  const tdStyle: React.CSSProperties = {
    padding: '9px 14px', fontSize: 12, borderBottom: '1px solid #f3f4f6',
  }
    ...
    <div id="monthly-report" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 10 }} className="report-summary-grid">
          {[
            ...
            <div key={c.label} style={{ padding: '14px 16px', background: c.bg, border: `1px solid ${c.bd}` }}>
              ...
              <p style={{ fontSize: 20, fontWeight: 800, color: c.color, marginTop: 4 }}>
                {c.value !== null ? `${formatCurrency(c.value)}원` : c.str}
              </p>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }} className="report-detail-grid">
        ...
        <div style={{ background: '#fff', border: '1px solid #e4e9f5', boxShadow: '0 1px 3px rgba(17,24,39,0.06)', padding: '20px 22px' }}>
          <p style={{ fontSize: 13, fontWeight: 800, color: '#374151', marginBottom: 14, borderBottom: '2px solid #059669', paddingBottom: 6 }}>
            상세 수입 내역
          </p>
        ...
        {/* 상세 지출 */}
        <div style={{ background: '#fff', border: '1px solid #e4e9f5', boxShadow: '0 1px 3px rgba(17,24,39,0.06)', padding: '20px 22px' }}>
          <p style={{ fontSize: 13, fontWeight: 800, color: '#374151', marginBottom: 14, borderBottom: '2px solid #dc2626', paddingBottom: 6 }}>
            상세 지출 내역
          </p>
            ...
            {[
              ...
              return (
                <tr key={row.label}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: d.pos ? '#059669' : '#dc2626' }}>
                    {d.str}
                  </td>
                </tr>
              )
      ...
      <div style={{ background: 'linear-gradient(120deg,#f5f3ff,#eff6ff)', border: '1px solid #c7c3fa', padding: '20px 22px' }}>
        <p style={{ fontSize: 13, fontWeight: 800, color: '#374151', marginBottom: 14, borderBottom: '2px solid #7c3aed', paddingBottom: 6 }}>
          {nextY}년 {nextM}월 예측
          <span style={{ fontWeight: 600, color: '#9ca3af', fontSize: 11, marginLeft: 8 }}>
            최근 {Math.min(3, allStats.length)}개월 추세 기반
          </span>
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            ...
            { label: '예상 순수익', value: pred.balance,
              color:  pred.balance >= 0 ? '#4f46e5' : '#dc2626',
              bg:     pred.balance >= 0 ? '#f5f3ff' : '#fef2f2',
              bd:     pred.balance >= 0 ? '#c7c3fa' : '#fca5a5' },
          ].map(c => (
            <div key={c.label} style={{ padding: '16px', background: c.bg, border: `1px solid ${c.bd}` }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: '#6b7280' }}>{c.label}</p>
    ...
    <div className="page-wrap space-y-5 sm:space-y-6">
      ...
      <div>
        ...
        <div className="no-print fade-up" style={{
          ...
          borderRadius: 16, boxShadow: 'var(--day-shadow)', padding: '20px 24px',
          marginBottom: 16,
        }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <select
                ...
                style={{
                  ...
                  border: '1px solid var(--day-border)', borderRadius: 10,
                  background: 'var(--day-card)', color: 'var(--day-text1)', cursor: 'pointer',
                }}
              >
                {monthOptions.map(o => (
